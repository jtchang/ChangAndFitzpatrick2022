...
def parse_header_mat(filename):
    with h5py.File(filename, 'r') as f:
        if 'header' in f:
            return _unpack_struct(f['header'], f)
        else:
            EOFError('Mat File does not containe header information')


def parse_header_json(filename):
    with open(filename) as f:
        return _unpack_json(json.load(f))


def _unpack_struct(struct, filehandle):
    struct_dict = {}
    for key, val in struct.items():
        if isinstance(val, h5py.Group):
            struct_dict[key] = _unpack_struct(val, filehandle)
        else:
            unpacked_val = val[()]
            if unpacked_val.size == 1:
                unpacked_val = unpacked_val[0][0]
            else:
                unpacked_val = np.squeeze(unpacked_val)
                for idx, element in enumerate(unpacked_val):
                    if isinstance(element, h5py.Reference):
                        ref_val = filehandle[element][()]
                        ref_val = ref_val[0][0] if ref_val.size == 0 else np.squeeze(ref_val)
                        unpacked_val[idx] = ref_val
            struct_dict[key] = unpacked_val

    return struct_dict


def _unpack_json(json_contents):
    struct_dict = {}

    for key, val in json_contents.items():
        subkeys = key.split('.')
        if subkeys[0] not in struct_dict.keys():
            struct_dict[subkeys[0]] = {}

        if len(subkeys) == 2:
            struct_dict[subkeys[0]][subkeys[1]] = val
        elif len(subkeys) == 3:
            if subkeys[1] not in struct_dict[subkeys[0]].keys():
                struct_dict[subkeys[0]][subkeys[1]] = {}
            struct_dict[subkeys[0]][subkeys[1]][subkeys[2]] = val
    return struct_dict
